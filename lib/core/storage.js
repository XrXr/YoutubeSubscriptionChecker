...
function update_last_check(trans, cb = util.noop, now = (new Date()).getTime()) {
    ...
    open_req.onsuccess = () => {
        ...
        if (!just_populated) {
            ...
            if (idb.cmp(db.objectStoreNames, STORE_NAMES) !== 0) {
                console.error("Not all object store present and db is not new. DB corrupted?");
                db.close();
