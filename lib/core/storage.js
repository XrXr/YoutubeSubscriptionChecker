...
const video = {
    ...
    put_into_history(trans, to_remove, cb) {
        ...
        // check if it's in video store
        let cursor_req = vs.openCursor(id);
        cursor_req.onsuccess = () => {
            let cursor = cursor_req.result;
            if (cursor) {
                let del_req = vs.delete(id);
                del_req.onsuccess = update_history;
        ...
        function update_history() {
            let hs = history_store(trans);
            // TODO: make sure to not put Angular $$hashkey into the db
            let req = hs.put(to_remove);
...
const channel = {
    ...
    add_one(trans, channel, cb) {
        ...
        let done = false;
        let count = 2;
        let add_req = channel_store(trans).add(channel);
        let stamp_req = check_stamp.update(trans, channel.id, Date.now());

        add_req.onsuccess = success_cb;
        stamp_req.onsuccess = success_cb;

        add_req.onerror = error_cb;
        stamp_req.onerror = error_cb;
...
const filter = {
    get_for_channel(trans, id, cb) {
        let req = filter_store(trans).index("channel_id").openCursor();
        let video_filters = [];
        req.onsuccess = ev => {
            let cursor = ev.target.result;
            if (cursor) {
                video_filters.push(cursor.value);
                cursor.continue();
            } else {
                cb(null, video_filters);
            }
        };
...
function initialize_db(cb) {
    ...
    open_req.onupgradeneeded = () => {
        ...
        video.createIndex("upload_time", "upload_time");
