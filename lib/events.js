...
function handle_basic_events (target) {
    ...
    let get_configs = get_db().transaction(["config", "filter"], "readonly");
    util.cb_join([done => config.get_all(get_configs, done),
                  done => done(null, [])],   // TODO: fill this after storage.filter.get_all
        (err, config, filters) => {
            if (err) {
    ...
    function remove_video (video, skip_opening) {
    ...
    target.on("remove-video", remove_video);
    target.on("skip-video", video => remove_video(video, true));
