...
function fetch_durations(pending_fetches) {
    for (let video_id of pending_fetches) {
        request.get_duration(video_id).then(duration_result => {
            ...
            storage.duration_fetch.remove_one(trans, video_id);
...
function check_all () {
    ...
    storage.channel.get_all(check, (err, channel_list) => {
        ...
        let promises = channel_list.map(channel => new Promise((resolve, reject) => {
            ...
            util.cb_each([function get_check_stamp(done) {
                storage.check_stamp.get_for_channel(check, channel.id, (err, stamp) => {
                    if (err) {
                        return done(err);
                    }
                    latest_date = stamp;
            ...
            }, function get_filters(done) {
                storage.filter.get_for_channel(check, channel.id, (err, video_filters) => {
                    if (err) {
                        return done(err);
                    }
...
function maybe_notify_new_upload(process_result) {
    ...
    for (let channel_id of process_result) {
        storage.channel.get_by_id(trans, channel_id, (_, channel) => {
...
function init() {
    if ("YTCHECKERDEBUG" in require("sdk/system").env) {
        require("./development").run(actual_init);
    } else {
        actual_init();
    }
}

function actual_init() {
