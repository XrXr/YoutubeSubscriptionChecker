...
function process_channel_activities (response_json, cb) {
    ...
    let trans = db.transaction(["video", "history", "check_stamp", "filter", "duration_fetch"], "readwrite");
...
if (self.loadReason === "upgrade") {
    ...
    if (migration.v1InUse()) {
        migration.v1Tov3(init_and_show_changelog);
    } else if (migration.v2InUse()) {
        migration.v2Tov3(init_and_show_changelog);
    } else {
